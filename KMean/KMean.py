...
def binKMeans(dataSet, k, distMethod=distEclud):
    ...
    while (len(centers) < k):
        ...
        for i in range(len(centers)):
            ptsInCurrCluster = dataSet[nonzero(clusterAssess[:][0].A == i)[0]][:]  # get the data points currently in cluster i
            spiltedCenter, spiltedAssess = kMeans(ptsInCurrCluster, 2, distMethod)
            spiltedError = sum(spiltedAssess[:][1])  # compare the SSE to the currrent minimum
            formerError = sum(clusterAssess[nonzero(clusterAssess[:][0].A != i)[0]][1])
            if (spiltedError + formerError) < minError:
                bestCentToSplit = i
                bestNewCents = spiltedCenter
                bestClustAss = spiltedAssess.copy()
                minError = spiltedError + formerError
        # update assessment
